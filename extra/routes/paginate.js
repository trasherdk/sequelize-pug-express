...
exports.hasNextPages = function hasNextPages (req) {
  return function (pageCount) {
    if (typeof pageCount !== 'number' || pageCount < 0) { throw new Error('express-paginate: `pageCount` is not a number >= 0') }
...
exports.getArrayPages = function (req) {
  return function (limit, pageCount, currentPage) {
    ...
    if (typeof limit !== 'number' || limit < 0) { throw new Error('express-paginate: `limit` is not a number >= 0') }

    if (typeof pageCount !== 'number' || pageCount < 0) { throw new Error('express-paginate: `pageCount` is not a number >= 0') }

    currentPage = parseInt(currentPage, 10)
    if (Number.isNaN(currentPage) || currentPage < 0) { throw new Error('express-paginate: `currentPage` is not a number >= 0') }
